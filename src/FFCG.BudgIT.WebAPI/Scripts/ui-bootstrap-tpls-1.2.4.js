...
.controller('UibCarouselController', ['$scope', '$element', '$interval', '$timeout', '$animate', function($scope, $element, $interval, $timeout, $animate) {
  ...
  self.select = $scope.select = function (nextSlide, direction) {
      ...
      // SCROLL WINDOW TO TOP AS WELL - EDIT BY DANIEL TIBBING
      window.scrollTo(0, 0);
      /*
      $("html, body").animate(
          { scrollTop: "0" },
  ...
  $scope.$watch('active', function(index) {
    if (angular.isNumber(index) && currentIndex !== index) {
      ...
      if (slide) {
        setActive(index);
        self.select(slides[index]);
        //MOVED THIS DOWN SO ANIMATION WORKS PROPERLY - EDIT BY DANIEL TIBBING
        currentIndex = index;
      }
    }
  });
...
angular.module("uib/template/datepicker/popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/datepicker/popup.html",
    ...
    "        <button type=\"button\" class=\"btn btn-sm btn-info datepicker-button uib-datepicker-current\" ng-click=\"select('today', $event)\" ng-disabled=\"isDisabled('today')\">{{ getText('current') }}</button>\n" +
    "        <button type=\"button\" class=\"btn btn-sm btn-danger datepicker-button uib-clear\" ng-click=\"select(null, $event)\">{{ getText('clear') }}</button>\n" +
    "      </span>\n" +
    "      <button type=\"button\" class=\"btn btn-sm btn-success datepicker-button pull-right uib-close\" ng-click=\"close($event)\">{{ getText('close') }}</button>\n" +
    "    </li>\n" +
    "  </ul>\n" +
    "</div>\n" +
    "");
